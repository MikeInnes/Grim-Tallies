...
# Notes:
# We collected the sero-surveys from medical journals, government websites, or,
# in a few instances, press reports (which were triangulated). All were
# conducted by medical researchers and/or government agencies. We only included
# serosurveys taken in general populations (i.e. not only health-care workers).
# Because sero-surveys are conducted over an interval of time, one has to make a
# decision about how to match it to case tallies. We decided to use the midpoint
# of the period in which samples were collected. Because sero-surveys measure
# the prevalence of antibodies rather than active infections, they must be
# brought backwards in time to correspond to percentages of those with a past or
# current likely active infection. We selected an interval of two weeks. As we
# discuss in the briefing, sero-surveys may not reflect the number of people who
# have been infected with covid-19. In some world regions, the circulation of
# viruses similar to covid-19 may trigger antibody-tests, even if a person has
# not been infected. Tests may suffer from poor sensitivity, or specificity
# (where available, we used results which corrected for this). Test subjects may
# not be fully random, and methods to make them closer to random or as-if random
# (through e.g. re-weighting by demographics), which we used were available, may
# not always be successful. Finally, the extent to which covid-19 produces
# antibodies was at the time of writing not fully understood. It may be that
# some or many, especially those who have suffered only mild infections, do not
# produce antibodies, or do so to an extent that is picked up by antibody tests.

# NB: As we make clear in the chart footnote and briefing text, this model
# extrapolates from data in places where serosurveys are available to places
# they are not. This means they must be taken with more than a grain of salt.
# Some countries may not fit the pattern. There are fewer available serosurveys
# from countries with lower average incomes, and fewer serosurveys from recent
# dates. This means that all these estimates are very uncertain, especially for
# times and places where serosurveys are less available. It may be that patterns
# found among these serosurveys and cases and deaths do not extrapolate well. It
# is however crucial to get better estimates of the scale of the pandemic than
# those provided by official case counts alone, which as of September 15
# suggested 30m cases worldwide. This presents one effort to do so with the data
# limitations we have. To improve our sense of the scale of the pandemic, more
# serosurveys conducted according to scientific standards, especially in
...
# Notes:
# GDP per capita is at the national level, case_ir and death_r are for the
# geographical units in which the serosurvey was conducted (national,
# province/state, region, or metropolitan level). In a few very rare instances,
# a sero-survey was conducted across a large geographical region within a
# country which could plausibly similar to the country average, e.g. Niger state
# in Nigeria, where no case or death data was available. In these instances, we
# used data from the national level instead. Smaller surveys (e.g. cities) which
...
# rates and their interaction, scaled by a proxy for testing capacity
# (log_gdp_per_capita). Furthermore, it assumes that as diagnosed deaths and
# cases rise as a share of population, they come to better reflect underlying
# cases and deaths (hence taking the natural logs of both). We weight
# observations by the log of sample size, so that larger serosurveys, which have
# less uncertainty, are more influential. If available for all countries, we
# would have preferred to use tests conducted and their positive rate in these
# models. However, while this data is available for many countries (Our World in
# Data provides an excellent resource), they are often incomparable and more
# importantly, not available for all countries. Furthermore, even where they are
# available, they are usually not available throughout the pandemic, especially
# in the crucial early months. Diagnostics of this model revealed Afghanistan's
# national survey to be especially high-leverage. We therefore contacted the WHO
# to confirm the estimate provided of seroprevalence in that survey. The WHO
# Press Office confirmed it was the correct preliminary estimate. Many other
# modelling strategies are possible, some of which we tried (e.g. beta
# regression and a random forest), their estimates were similar and landed
# within the interval of our predictions. A linear model was thought to be the
# most straightforward, interpretable, and conservative choice.
...
# average to avoid day-to-day variation driving changes.
library(runner)
...
# To generate the large plot, we first-differences and by continent and a select
# few large countries:
...
# This function takes first differences by country and sums countries together
# by day and group:
...
# Notes: As we write and show through our confidence interval above, newer dates
# are less certain, as sero-surveys are only released some time after data is
# collected. The labels and order in which groups appear and other visual
# details were tweaked by our graphical designers.
